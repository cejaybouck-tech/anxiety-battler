  ...
  <main class="min-h-screen py-6 md:py-10">
    ...
    >
      ...
      >
        <div>
          ...
          <h1 class="mt-3 text-2xl font-semibold tracking-tight md:text-3xl">
            Welcome back,
            <span class="text-sky-600">{{ player.username }}</span>
          </h1>
          <p class="mt-1 text-sm text-slate-600">
            Here&apos;s a quick look at how your anxiety has been doing
            recently.
          </p>
        </div>
      ...
      <div class="grid gap-6 md:grid-cols-3">
        <!-- Anxiety trend card -->
        <UCard
          v-if="player.checkins && player.checkins.length > 0"
          class="md:col-span-2"
        ...
        >
          <div class="mb-4 flex items-center justify-between gap-2">
            <div>
              ...
              <p class="text-xs">
                One check-in per day, last 7 days (example data)
              </p>
            </div>
            <div class="text-right text-xs">
              <p>
                Avg ({{ currentTimeframeLabel }}):
                <span class="font-semibold">{{ averageAnxiety }}</span>
              </p>
              <p class="mt-0.5">
                Latest:
                <span
                  :class="latestAnxietyClass"
                  class="ml-1 inline-flex items-center rounded-full px-2 py-0.5 text-[0.7rem]"
                >
        ...
        <UCard v-else class="md:col-span-2" aria-label="Anxiety trend"
          >You have no checkins saved</UCard
        >
        ...
        <UCard aria-label="User overview">
          ...
          <div class="space-y-3 text-xs">
            ...
            <div class="flex items-center justify-between">
              ...
              <span class="font-medium"
                >{{ player.experience }} / {{ expToNextLevel }}</span
              >
            </div>
            ...
            <div class="flex items-center justify-between">
              <span>Gold</span>
              <span
                class="inline-flex items-center gap-1 font-semibold text-amber-700"
              >
          ...
          <p class="mt-1 text-[0.7rem]">
            Every check-in and completed &quot;attack&quot; earns experience and
            gold over time.
          </p>
        </UCard>
      </div>
      ...
      <div class="">
        ...
        <UCard aria-label="Upcoming event">
          <template #header class="flex justify-between gap-2">
            <div>
              <h2 class="text-sm font-semibold">Next upcoming battle</h2>
              <p class="text-xs">
                Something you&apos;re feeling anxious about and will overcome!
              </p>
            </div>
          </template>
          ...
          <div v-if="upcomingEvent" class="flex gap-4">
            ...
            <div class="space-y-2 text-xs">
              ...
              <p class="text-[0.7rem]">
                {{
                  formatDateRange(
                    upcomingEvent.dateStart,
                    upcomingEvent.dateEnd
                  )
                }}
                · Anxiety attached:
                <span class="font-semibold text-slate-800"
                  >{{ upcomingEvent.anxiety }} / 100</span
                >
              </p>
              <UBadge
                v-if="upcomingEvent"
          ...
          <div
            v-if="upcomingEvent"
            class="mt-1 flex flex-wrap items-center gap-1.5 text-[0.7rem]"
          >
            <UCard
              v-for="(mob, index) in upcomingEvent.mobs"
              :key="mob.title + ' ' + index"
              class="w-[225px] h-[200px] flex flex-col gap-2 rounded-2xl border-slate-100 bg-slate-50/70 p-3 md:flex-row md:items-center md:justify-between"
            >
              <div class="space-y-1">
...
const latestCheckin = computed(() => {
  if (
    !player.value ||
    !player.value.checkins ||
    player.value.checkins.length < 1
  )
    return undefined;
...
const expProgress = computed(() => {
  ...
  return Math.min(
    100,
    Math.round((player.value?.experience / expToNextLevel.value) * 100)
  );
});
...
const upcomingEvent = computed<Boss | undefined>(() => {
  ...
  const closestEvent = player.value.bosses.reduce((prev, curr) => {
    const diffPrev = Math.abs(
      new Date(prev.dateStart).getTime() - today.getTime()
    );
    const diffCurr = Math.abs(
      new Date(curr.dateStart).getTime() - today.getTime()
    );
